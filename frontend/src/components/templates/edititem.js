...
const EditItem = (props) => {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:4000/api/user/itemeditget", {
      ...
      .then((response) => {
        ...
        setFields(response.data.addons);
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("http://localhost:4000/api/user/edititem", {
        details: formData,
        id: localStorage.getItem("Item"),
      })
      .then((response) => {
        alert(
          "Updated item " + name + " for vendor " + localStorage.getItem("Name")
        );
        console.log(response.data);
      });
